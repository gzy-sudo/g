...
import torch
# 将第10行改为：
from .image_utils import crop_img, random_augmentation
...
class PromptTrainDataset(Dataset):
    ...
    def _init_rs_ids(self):
        temp_ids = []
        rs = self.args.data_file_dir + "input/rainTrain.txt"
    ...
    def _get_gt_name(self, rainy_name):
        ...
        # 如果rainy_name只是文件名，添加基础路径
        if not ('/' in rainy_name or '\\' in rainy_name):
            # 只是文件名，如 'rain-176.png'
            rainy_name = os.path.join(self.args.derain_dir, 'rainy', rainy_name)

        # 确保路径标准化
        rainy_name = os.path.normpath(rainy_name)

        # 提取目录和文件名
        dir_path = os.path.dirname(rainy_name)  # 如：'data/Train/Derain/rainy'
        file_name = os.path.basename(rainy_name)  # 如：'rain-176.png'

        # 获取父目录
        parent_dir = os.path.dirname(dir_path)  # 'data/Train/Derain'

        # 构建gt路径
        # 尝试不同的gt目录名
        gt_dirs = ['gt', 'target', 'clear', 'clean', 'GT']
        clean_filename = file_name.replace('rain-', 'norain-')

        for gt_dir in gt_dirs:
            gt_path = os.path.join(parent_dir, gt_dir, clean_filename)
            if os.path.exists(gt_path):
                return gt_path

        # 如果都没找到，返回默认路径
        default_gt = os.path.join(parent_dir, 'gt', clean_filename)
        print(f"⚠️  警告：使用默认路径 {default_gt}，文件可能不存在")
        return default_gt
