...
def prepare_video(video_path, max_frames=None, resize=None):
    """
    读取视频并准备为tensor
    ...
    if resize is not None:
        video = torch.nn.functional.interpolate(
            video.permute(0, 2, 3, 1),  # [T, H, W, C]
            size=resize,
            mode='bilinear',
            align_corners=False
        ).permute(0, 3, 1, 2)  # 回到 [T, C, H, W]

    return video.float() / 255.0  # 归一化到 [0, 1]


def save_video(video_tensor, output_path, fps=30):
    """
    保存视频tensor为文件

    参数:
        video_tensor: [T, C, H, W] 或 [T, H, W, C]，值在[0, 1]或[0, 255]
        output_path: 输出文件路径
        fps: 帧率
    """
    # 确保tensor在CPU上
    ...
    if video_tensor.max() <= 1.0:
        video_tensor = (video_tensor * 255).clamp(0, 255)

    # 确保数据类型是uint8
    video_tensor = video_tensor.byte()

    # 确保维度顺序是 [T, H, W, C]
    if video_tensor.dim() == 4 and video_tensor.shape[1] == 3:  # [T, C, H, W]
